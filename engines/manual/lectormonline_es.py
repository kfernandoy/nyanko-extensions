...
cada una donde es mejor:

* ``/api/comics`` -- API REST limpia y paginada (5343 series) para listar y buscar. El
  parametro de busqueda es ``search``; con ``title`` o ``q`` responde 200 pero devuelve el
  catalogo entero sin filtrar.
* ``<ruta>/__data.json`` -- el payload de SvelteKit, para la ficha y el lector.

Ese payload viene en formato aplanado (estilo devalue): ``data`` es un array donde cada
valor es el INDICE de otro elemento del mismo array, no el valor en si. Por eso hace falta
``_resolver``; leerlo como JSON normal da numeros sueltos en lugar de los campos.
"""
...
SOURCE = GeneratedGatoLibreriaSource

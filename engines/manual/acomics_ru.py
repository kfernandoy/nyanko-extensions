...
class GeneratedGenericSource(GenericSource):
    name = 'acomics_ru'
    display_name = 'AComics'
    base_url = 'https://acomics.ru'
    language = 'ru'
    requests_per_minute = 60
    ...
    def get_filters(self) -> list[SourceFilter]:
        return [
            # Ratings
            SourceFilter("rating_1", "NR (Возрастная категория)", "checkbox", default=True),
            SourceFilter("rating_2", "G (Возрастная категория)", "checkbox", default=True),
            SourceFilter("rating_3", "PG (Возрастная категория)", "checkbox", default=True),
            SourceFilter("rating_4", "PG-13 (Возрастная категория)", "checkbox", default=True),
            SourceFilter("rating_5", "R (Возрастная категория)", "checkbox", default=True),
            SourceFilter("rating_6", "NC-17 (Возрастная категория)", "checkbox", default=True),
            
            # Sort
            SourceFilter("sort", "Сортировка", "sort", options=[
                ("last_update", "по дате обновления"),
                ("subscr_count", "по количеству подписчиков"),
                ("issue_count", "по количеству выпусков"),
                ("serial_name", "по алфавиту"),
            ], default="subscr_count"),

            # Type
            SourceFilter("type", "Тип комикса", "select", options=[
                ("0", "Все"),
                ("orig", "Оригинальный"),
                ("trans", "Перевод"),
            ], default="0"),
            ...
            # Publication
            SourceFilter("publication", "Публикация", "select", options=[
                ("0", "Все"),
                ("no", "Завершенный"),
                ("yes", "Продолжающийся"),
            ], default="0"),
            ...
            # Subscription
            SourceFilter("subscription", "Подписка", "select", options=[
                ("0", "Все"),
                ("yes", "В моей ленте"),
                ("no", "Кроме моей ленты"),
            ], default="0"),
            ...
            # Min pages
            SourceFilter("min_pages", "Минимум страниц", "text", default="2"),

            # Categories
            SourceFilter("cat_1", "Животные (Категория)", "checkbox", default=False),
            SourceFilter("cat_2", "Драма (Категория)", "checkbox", default=False),
            SourceFilter("cat_3", "Фентези (Категория)", "checkbox", default=False),
            SourceFilter("cat_4", "Игры (Категория)", "checkbox", default=False),
            SourceFilter("cat_5", "Юмор (Категория)", "checkbox", default=False),
            SourceFilter("cat_6", "Журнал (Категория)", "checkbox", default=False),
            SourceFilter("cat_7", "Паранормальное (Категория)", "checkbox", default=False),
            SourceFilter("cat_8", "Конец света (Категория)", "checkbox", default=False),
            SourceFilter("cat_9", "Романтика (Категория)", "checkbox", default=False),
            SourceFilter("cat_10", "Фантастика (Категория)", "checkbox", default=False),
            SourceFilter("cat_11", "Бытовое (Категория)", "checkbox", default=False),
            SourceFilter("cat_12", "Стимпанк (Категория)", "checkbox", default=False),
            SourceFilter("cat_13", "Супергерои (Категория)", "checkbox", default=False),
            SourceFilter("cat_14", "Детектив (Категория)", "checkbox", default=False),
            SourceFilter("cat_15", "Историческое (Категория)", "checkbox", default=False),
        ]

    async def search(self, query: str, page: int = 1, filters: dict | None = None) -> list[SourceSeries]:
        if not filters:
            filters = {f.id: f.default for f in self.get_filters()}
        ...
        if query.strip():
            ...
            ratings = []
            for i in range(1, 16):
                if filters.get(f"cat_{i}"):
                    categories.append(str(i))
            for i in range(1, 7):
                if filters.get(f"rating_{i}"):
            ...
            params["type"] = filters.get("type", "0")
            params["upd"] = filters.get("publication", "0")
            params["subscribe"] = filters.get("subscription", "0")
            params["issue_count"] = filters.get("min_pages", "2")
            params["sort"] = filters.get("sort", "subscr_count")
        ...
        try:
            ...
            result = []
            for item in root.descendants():
                if item.tag == "section" and item.has_class("serial-card"):
                    anchor = _first(item, lambda node: node.tag == "a" and bool(node.attrs.get("href")))
                    if not anchor: continue
                    title_box = _first(item, lambda node: node.tag == "h2" and node.parent == item)
                    if not title_box: continue
                    title_anchor = _first(title_box, lambda node: node.tag == "a")
                    if not title_anchor: continue
                    
                    source_id = urljoin(f"{self.base_url}/", title_anchor.attrs.get("href", "")) + "/about"

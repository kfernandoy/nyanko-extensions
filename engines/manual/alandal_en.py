...
class GeneratedGenericSource(GenericSource):
    name = 'alandal_en'
    display_name = 'Alandal'
    base_url = 'https://alandal.com'
    language = 'en'
    requests_per_minute = 60
    ...
    def get_filters(self) -> list[SourceFilter]:
        return [
            SourceFilter(
                ...
                options=[
                    {"value": "1", "name": "Action"},
                    {"value": "2", "name": "Fantasy"},
                    {"value": "3", "name": "Regression"},
                    {"value": "4", "name": "Overpowered"},
                    {"value": "5", "name": "Ascension"},
                    {"value": "6", "name": "Revenge"},
                    {"value": "7", "name": "Martial Arts"},
                    {"value": "8", "name": "Magic"},
                    {"value": "9", "name": "Necromancer"},
                    {"value": "10", "name": "Adventure"},
                    {"value": "11", "name": "Tower"},
                    {"value": "12", "name": "Dungeons"},
                    {"value": "13", "name": "Psychological"},
                    {"value": "14", "name": "Isekai"},
                ],
                default=""
            ),
            SourceFilter(
    ...
    async def search(self, query: str, filters: dict | None = None, page: int = 1) -> list[SourceSeries] | dict:
        filters = filters or {}
        api_url = "https://qq.alandal.com/api/series"
        ...
        params["sort"] = filters.get("sort") or "popular"
        params["status"] = filters.get("status") or "-1"
        ...
        response = await self._request("GET", api_url, params=params, headers=headers)
        
        # The base GenericSource class parses JSON using _adaptive_series automatically if it receives JSON.
        # However, Alandal's API response is nested: {"data": {"series": {"data": [...]}}}
        try:
            payload = response.json()
            data_list = payload.get("data", {}).get("series", {}).get("data", [])
            result = []
            from urllib.parse import urljoin
            for item in data_list:
                item_url = item.get("url") or item.get("slug")
                title = item.get("title") or item.get("name")
                cover = item.get("cover") or item.get("thumbnail")
                if item_url and title:
                    source_id = urljoin(f"{self.base_url}/series/", str(item_url))
                    result.append(
                        SourceSeries(
                            source_id=source_id,
                            title=str(title),
                            source_name=self.name,
                            cover_url=urljoin(self.base_url, cover) if cover else None,
                            web_url=source_id
                        )
                    )
            return result

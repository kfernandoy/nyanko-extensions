...
class GeneratedGroupLeSource(GroupLeSource):
    ...
    def get_filters(self) -> list[SourceFilter]:
        return [
            SourceFilter(
                ...
                name="Sort By",
                options=[
                    {"value": "rate", "name": "По популярности"},
                    {"value": "updated", "name": "По дате обновления"},
                    {"value": "name", "name": "По алфавиту"},
                    {"value": "created", "name": "По дате добавления"},
                    {"value": "chapter_count", "name": "По количеству глав"}
                ],
                default="rate"
            ),
            SourceFilter(
    ...
    async def search(self, query: str, filters: dict | None = None, page: int = 1) -> list[SourceSeries]:
        filters = filters or {}
        
        url = f"{self.base_url}/search/advancedResults"
        params = {
            "offset": str(50 * max(page - 1, 0))
        }
        ...
        sort_type = filters.get("sortType")
        if sort_type:
            params["sortType"] = sort_type
            
        # GroupLeuses "=in" for included states.
        # We assume the UI sends a list of included values for group/checkbox filters.
        for filter_group in ["categories", "genres", "additional"]:
            selected = filters.get(filter_group)
            if selected:
                for val in selected:
                    params[val] = "=in"

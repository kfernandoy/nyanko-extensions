...
class GeneratedGenericSource(GenericSource):
        ...
        return [
            ...
            SourceFilter(
                ...
                options=[
                    ...
                    {"value": "Crossdressing", "name": "Crossdressing"},
                    {"value": "Dementia", "name": "Dementia"},
                    {"value": "Demons", "name": "Demons"},
    ...
    async def search(self, query: str, filters: dict | None = None, page: int = 1) -> list[SourceSeries] | dict:
        ...
        allow_adult = prefs.get("pref_adult", False)
        ...
        if sort_by:
            search_payload["sortBy"] = sort_by
            
        # Genre is complex in Python because we don't have TriState natively yet,
        # but if passed as lists for included/excluded we can parse them.
        # Assuming our client passes a list of strings if it's a checkbox group.
        # For simplicity, if we get a list from 'genres' we assume it's included.
        genres = filters.get("genres")
        if genres:
            search_payload["genres"] = genres
        ...
        variables = {
            ...
            "page": page,
            "translationType": "sub",
            "countryOrigin": filters.get("country", "ALL")
        }
        ...
        response = await self._request("POST", "https://api.mkissa.net/api", json=payload, headers=headers)
        ...
        try:
            ...
            for item in edges:
                id_ = item.get("_id")
                name = item.get("englishName") or item.get("name")
                ...
                if id_ and name:
                    result.append(
                        SourceSeries(
                            ...
                            source_name=self.name,
                            cover_url=thumbnail,
                            web_url=f"https://allanime.to/manga/{id_}"
                        )
                    )
            return result
        except Exception:
    ...
    async def browse(self, kind: str, page: int = 1) -> list[SourceSeries]:
        ...
        try:
            data = response.json()
            recs = data.get("data", {}).get("queryPopular", {}).get("recommendations", [])
            result = []
            for rec in recs:
                item = rec.get("anyCard")
    ...
    async def chapters(self, series: SourceSeries | str) -> list[SourceChapter]:
        raise NotImplementedError("GraphQL + WebView needed")
        
    async def pages(self, chapter: SourceChapter | str) -> list[SourcePage]:
        raise NotImplementedError("WebView needed to decrypt pages")

...
"""Implementación de Traducciones Moonlight.

El sitio es un Next.js cuyo HTML no sirve el catalogo paginado: ``/comics?page=2`` devuelve
exactamente las mismas 15 series que la primera pagina, asi que raspar el DOM solo daba una
porcion fija. Por debajo hay una API JSON sin autenticacion:

* ``/api/comics`` -- las 439 series de una vez (1,2 MB). No pagina; se reparte en el cliente.
* ``/api/showProject/{slug}`` -- ficha con ``lastChapters``, que pese al nombre trae la lista
  completa de capitulos.
* ``/api/showProject/{slug}/{chapter_slug}`` -- el capitulo, con las paginas.

Cuidado con ``pages.urlImg``: es un STRING que contiene un JSON, no una lista. Hay que
deserializarlo aparte o se interpreta como una cadena suelta.
"""
...
SOURCE = GeneratedMoonlightSource

...
"""Adaptador de Manhwa Scan: API propia con ids "<id>#<slug>"."""

_MANTRAZ_GENRES = (
    ("", "Todos"), ("accion", "Acción"), ("romance", "Romance"), ("fantasia", "Fantasía"),
    ("drama", "Drama"), ("comedia", "Comedia"), ("thriller", "Thriller"),
    ("artes-marciales", "Artes Marciales"), ("harem", "Harem"), ("isekai", "Isekai"),
    ("horror", "Horror"), ("vida-escolar", "Vida Escolar"), ("seinen", "Seinen"),
    ("shounen", "Shounen"),
)


class ManhwaScanSource(MadaraSource):
    """Los ids viejos no llevan almohadilla y se migran buscando por titulo."""
    ...
    def get_filters(self) -> list[SourceFilter]:
        return [
            SourceFilter("genre", "Género", "select", list(_MANTRAZ_GENRES), ""),
            SourceFilter("status", "Estado", "select", [
                ("", "Todos"), ("ongoing", "En curso"),
                ("completed", "Completado"), ("hiatus", "Hiatus"),
            ], ""),
            SourceFilter("sort", "Ordenar", "select", [
                ("updated", "Recientes"), ("views", "Vistas"), ("rating", "Rating"),
            ], "updated"),
            SourceFilter("type", "Tipo", "select", [
                ("", "Todos"), ("adult", "Adulto"), ("bl", "BL"),
                ("shoujo", "Shoujo"), ("manga", "Manga"),
            ], ""),
        ]
    ...
    async def browse(self, kind: str, page: int = 1):
        ...
        payload = await self._get(f"{self.api_url}/series", [
            ("page", str(page)), ("limit", "48"),
            ("sort", "views" if kind == "popular" else "updated"), ("q", ""),
        ])
    ...
    async def search(self, query: str, page: int = 1, filters: dict | None = None):
        ...
        params: list[tuple[str, str]] = [
            ("page", str(page)), ("limit", "48"), ("q", query.strip()),
            ("sort", str(values.get("sort") or "updated")),
        ]
        params.extend(
            (key, str(values[key]))
        ...
        return self._browse(await self._get(f"{self.api_url}/series", params))

    async def details(self, series: SourceSeries | str) -> SourceSeries:
        series_id = series.source_id if isinstance(series, SourceSeries) else str(series)
        identifier = await self._identifier(series, series_id)
        payload = await self._get(f"{self.api_url}/series/{identifier}", [])
        data = ((payload.get("data") or {}).get("series")) or {}
        genres = [str(value) for value in data.get("genres") or []]
        return SourceSeries(
            source_id=f"{data.get('id')}#{data.get('slug')}",
            title=str(data.get("title") or ""),
            source_name=self.name,
            cover_url=self._absolute(data.get("cover_url")),
            ...
            web_url=f"{self.base_url}/manga/{data.get('slug')}/",
        )

    async def chapters(self, series: SourceSeries | str) -> list[SourceChapter]:
        series_id = series.source_id if isinstance(series, SourceSeries) else str(series)
        identifier = await self._identifier(series, series_id)
        payload = await self._get(f"{self.api_url}/series/{identifier}/chapters", [])
        result: list[SourceChapter] = []
        for item in ((payload.get("data") or {}).get("chapters")) or []:
            if not isinstance(item, dict):
                continue
            number = str(item.get("chapter_num") or "")
            label = number[:-2] if number.endswith(".0") else number
            title = str(item.get("title") or "").strip()
            result.append(
                SourceChapter(
                    source_id=f"{item.get('id')}#{item.get('slug')}",
                    title=f"Capítulo {label}" + (f" - {title}" if title else ""),
                    series_id=series_id,
                    source_name=self.name,
                    number=self._float(number),
                    language=self.language,
                    uploaded_at=self._date(item.get("created_at")),
                )
            )
        return result
    ...
    async def pages(self, chapter: SourceChapter | str) -> list[SourcePage]:
        ...
        if "#" not in chapter_id:
            raise ValueError("Por favor, actualiza la lista de capítulos para leer este capítulo.")
        payload = await self._get(f"{self.api_url}/chapters/{chapter_id.partition('#')[0]}", [])
        images = (((payload.get("data") or {}).get("chapter")) or {}).get("pages") or []
        urls = [
            value
            for image in images
            if isinstance(image, dict) and (value := self._absolute(image.get("image_url")))
        ]
        return [
            SourcePage(
                source_id=value,
    ...
    async def _identifier(self, series: SourceSeries | str, series_id: str) -> str:
        if "#" in series_id:
            return series_id.partition("#")[0]
        # Formato viejo sin almohadilla: se migra buscando la serie por titulo.
        title = series.title if isinstance(series, SourceSeries) else series_id
        payload = await self._get(f"{self.api_url}/series", [("q", title)])
        needle = title.casefold()
        for item in ((payload.get("data") or {}).get("series")) or []:
            if isinstance(item, dict) and needle in str(item.get("title") or "").casefold():
                return str(item.get("id"))
        raise SourceNotFoundError(f"No se pudo migrar el manga '{title}'. Búscalo de nuevo.")
    ...
    def _browse(self, payload: dict) -> dict:
        ...
        return {
            "items": [
                SourceSeries(
                    ...
                    source_name=self.name,
                    cover_url=self._absolute(item.get("cover_url")),
                    web_url=f"{self.base_url}/manga/{item.get('slug')}/",
                )
                for item in data.get("series") or []
                if isinstance(item, dict)
            ],
            "has_more": int(data.get("page") or 0) < int(data.get("total_pages") or 0),
        }
    ...
    def _absolute(self, value: Any) -> str | None:
        ...
        if not text:
            return None
        return f"{self.base_url}{text}" if text.startswith("/") else text

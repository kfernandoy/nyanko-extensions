...
"""Adaptador de Plot Twist No Fansub: capitulos por AJAX sobre un tema Madara."""


_PLOT_MANGA_ID =re .compile (r"mnWpMangaId\s*=\s*(\d+)")


_PLOT_OLD_MANGA_ID =re .compile (r'"manga_id"\s*:\s*"(\d+)"')


_PLOT_HTML_TAG =re .compile (r"<[^>]*>")


_PLOT_DATE =re .compile (r"([A-Za-zÁÉÍÓÚáéíóúñÑ]+)\s+(\d{1,2}),?\s+(\d{4})")


_PLOT_NUMBER =re .compile (r"\d+(?:\.\d+)?")
